...
class Aoe2mapTest(TestCase):
    ...
    def compareWithValidationFile(self, output, suffix="", masking=None):
        ...
        with open(os.path.join(script_file_path, "snapshots", "output", filename), "w") as f:
            print(output, file=f)

        if not os.path.isfile(os.path.join(script_file_path, "snapshots", "validation", filename)):
            with open(os.path.join(script_file_path, "snapshots", "validation", filename), "w") as f:
                print(validation, file=f)
        else:
            with open(os.path.join(script_file_path, "snapshots", "validation", filename), "r") as f:
                validation = f.read()
...
class ChangelogTest(Aoe2mapTest):
